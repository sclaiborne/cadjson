...
enough. The script needs only build123d and reproduces the cadjson build (same solid), with
...
from cadjson.sketch import _pattern_locations, open_path_wire
...
class PythonExporter:
    ...
    def _shape_code(self, shape) -> str:
        ...
        if isinstance(shape, PathShape):
            from cadjson.sketch import _path
...
TEMPLATE = '''"""{name}: standalone build123d script generated by cadjson {version}.

Equivalent to `cadjson build` for this part. Selectors are resolved to concrete geometry
