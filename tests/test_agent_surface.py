...
def test_committed_schema_matches_models():
    committed = json.loads((ROOT / "schema" / "cadjson-0.1.schema.json").read_text(encoding="utf-8"))
    assert committed == json_schema(), "run: cadjson schema -o schema/cadjson-0.1.schema.json"


def test_dollar_schema_key_is_accepted():
    doc = load_document(EXAMPLES / "knob.json")
    assert doc.schema_uri.endswith("cadjson-0.1.schema.json")
...
def test_skill_exists_and_mentions_workflow():
    text = (ROOT / ".claude" / "skills" / "cadjson" / "SKILL.md").read_text(encoding="utf-8")
    assert text.startswith("---\nname: cadjson")
    for needle in ("cadjson validate", "cadjson build", "report.json", "cadjson info", "convex"):
