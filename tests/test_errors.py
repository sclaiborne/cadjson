...
from cadgen.build import build_document, parse_document
...
def test_reference_to_later_feature_is_rejected():
    with pytest.raises(CadgenError, match="not an earlier feature id"):
        parse_document(_doc([
...
def test_unknown_param_names_feature():
    ...
    with pytest.raises(CadgenError, match=r"\[box\].*unknown parameter 'depth'"):
        build_document(doc)


def test_cut_first_is_an_error():
    doc = parse_document(_doc([{**BOX, "op": "cut"}]))
    with pytest.raises(CadgenError, match="no solid yet"):
        build_document(doc)
