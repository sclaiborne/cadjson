...
from cadgen.init import init_repo, skill_text

ROOT = Path(__file__).resolve().parent.parent


def test_packaged_skill_matches_repo_skill():
    repo = (ROOT / ".claude" / "skills" / "cadgen" / "SKILL.md").read_text(encoding="utf-8")
    assert skill_text() == repo, "copy .claude/skills/cadgen/SKILL.md to cadgen/skill/SKILL.md"
...
def test_init_scaffolds_and_update_refreshes(tmp_path):
    ...
    assert {".claude/skills/cadgen/SKILL.md", ".gitignore", "README.md", "requirements.txt", "CLAUDE.md"} <= names
    ...
    (target / ".claude/skills/cadgen/SKILL.md").write_text("stale", encoding="utf-8")
    init_repo(target, update=True)
    assert (target / "README.md").read_text(encoding="utf-8") == "mine"
    assert (target / ".claude/skills/cadgen/SKILL.md").read_text(encoding="utf-8") == skill_text()
